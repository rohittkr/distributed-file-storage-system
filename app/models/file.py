...
class ContentObjectReplica(Base):
    __tablename__ = "content_object_replicas"
    __table_args__ = (
        UniqueConstraint(
            "content_object_id",
            "storage_node_id",
    ...
    storage_node = relationship(
        "StorageNode",
        back_populates="content_object_replicas",
    )
...
class Chunk(Base):
    ...
    version = relationship("FileVersion", back_populates="chunks")
    content_object = relationship(
        "ContentObject",
...
class StorageNode(Base):
    ...
    content_object_replicas = relationship(
        "ContentObjectReplica",
        back_populates="storage_node",
        cascade="all, delete-orphan",
    )
...
class ChunkReplica(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_chunk_replicas_status", "status"),
    )
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    ...
    storage_node = relationship("StorageNode", back_populates="replicas")
